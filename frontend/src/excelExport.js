...
export function exportInspectionReport(updates, title = 'Inspection_Report') {
  ...
  const headers = [
    ...
    'Comment Severity', 'Consultant Action', 'Update Text', 'Comments', 'Photos'
  ]
  ...
  const rows = updates.map(u => [
    ...
    u.update_text || '',
    u.comments || '',
    (u.inspection_photos || []).map(p => p.photo_url).join('\n') || 'None'
  ])
  ...
  ws['!cols'] = headers.map(h => ({
    wch: h === 'IR Title' ? 70 : h === 'Document No' ? 42 :
         h === 'Update Text' || h === 'Comments' ? 50 : h === 'Photos' ? 40 : 22
  }))
  ws['!rows'] = [{ hpt: 28 }, { hpt: 18 }, { hpt: 8 },
    { hpt: 20 }, ...rows.map(() => ({ hpt: 60 }))]
  ws['!pageSetup'] = { paperSize: 8, orientation: 'landscape', fitToWidth: 1, fitToPage: true }
